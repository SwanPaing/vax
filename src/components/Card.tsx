    ...
    <div className="flex flex-wrap gap-2 p-4">
      {items.map((item, index) => (
        <Link to={`${item.id}`} key={item.id || index}>
          <div className={`group w-[15em] h-[18em] my-2 text-center bg-zinc-800 rounded-br-2xl rounded-tl-2xl overflow-hidden hover:border-green-400 hover:border-2 shadow-md hover:shadow-green-400 transition-all duration-300 hover:scale-[1.03] rotate-y-30 hover:rotate-y-0 ${isPlaying === item.id ? "border-green-400 border-2 shadow-md shadow-green-400 text-green-400" : ""} `}>
